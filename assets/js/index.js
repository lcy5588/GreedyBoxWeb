		...
		if(data){
			...
			for(var i= 0,len = data.length;i < len; i++){
                ...
                html.push( "<div class='col-xs-12 col-sm-4 col-md-4 item'><div class='thumbnail'><div class='gifcontrol'>"
							...
							+"<p>刺激度:<input type='number' data-id='"+ val.id +"' name='excitablelevel' id='excitablelevel_item_"+ val.id+"' value='2' class='rating'/>"
							+"<span>"+ parseFloat(val.excitablelevel).toFixed(1) +"</span></p>"
							+"<p>舒适度:<input type='number' data-id='"+ val.id+"' name='comfortablelevel' id='comfortablelevel_item_"+ val.id +"' value='2' class='rating'/>"
							+"<span>"+ parseFloat(val.comfortablelevel).toFixed(1) +"</span></p>"
							+"<p>性感度:<input type='number' data-id='"+val.id +"' name='sexlevel' id='sexlevel_item_"+ val.id+"' value='3' class='rating'/><span>"+ parseFloat(val.sexlevel).toFixed(1) +"</span></p>"
